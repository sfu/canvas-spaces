...
class ManagerController < ApplicationController
  ...
  def login
    # the method by which to authenticate and get a token that will allow us to call the REST api
  end
  ...
  # List all groups in the special group set that belongs to the special account.
  ...
  def list_groups
    ...
    if @current_user.account.site_admin?
      groups = group_cat.groups.where("groups.workflow_state != 'deleted'").eager_load(:users)
    else
      # find all groups owned by the student
      groups = group_cat.groups.where("groups.workflow_state != 'deleted' and leader_id = ?", @current_user.id).eager_load(:users)
    end

    # After looking at all the options of restricting json output and taking into consideration that I include user-count in the result,
    # this code seemed the clearest.
    response = []
    groups.each do |g|
      response << {id: g.id, name: g.name, leader_id: g.leader_id, created_at: g.created_at, description: g.description, size: g.users.count}
    end

    render json: { success: true, count: groups.count, groups: response }
  end
  
  # Create a group.
  # If called by a non-admin user then the user will be made leader of the group. Any specified leader is ignored.
  ...
  def create_group
    ...
    join_type_param = params[:join_type] # string, either 'free to join' or 'invite only'
    ...
    group = acct.groups.create( name: name_param, group_category: group_cat, leader: leader, join_level: join_type, description: desc_param)
    render :json => group
  end
  ...
  def group_info
    ...
    group = group_cat.groups.where("groups.id = ?", group_id_param).eager_load(:users).first
    if group == nil
  ...
  def modify_group
    ...
    if group == nil
      ...
      if @current_user.account.site_admin? || group.leader_id == @current_user.id
        if desc_param && !description.blank?
          group.description = desc_param
        end
  ...
  # The leader of the group may add any user. TODO: Should this be allowed? What if the user doesn't want to be a member of the group?
  ...
  def set_leader
    ...
    if group == nil
      ...
      if @current_user.account.site_admin? || group.leader_id == @current_user.id
        ...
        group.save
        render json: { success: true, message: "Successfully changed leader." }
      else
        # doesn't have access to the group
        render json: { success: false, message: "Can't change leader. Not owner." }
      end
    end
  end
